...
const E2E_TEST_TAG: &str = "e2e-test-tag-assign";
...
#[traced_test]
async fn e2e_test_rover_tag_assign_missing_positional_args() {
    ...
    assert_that(&stderr).contains("<GRAPH_ID>");
...
#[tokio::test(flavor = "multi_thread")]
#[traced_test]
async fn e2e_test_rover_tag_assign_missing_resource_id(remote_supergraph_graph_id: String) {
    let mut cmd = Command::new(cargo::cargo_bin!("rover"));
    cmd.args(["tag", "assign", &remote_supergraph_graph_id, E2E_TEST_TAG]);
    let output = cmd.output().expect("Could not run command");
...
#[tokio::test(flavor = "multi_thread")]
#[traced_test]
async fn e2e_test_rover_tag_assign_mutually_exclusive_resource_id(
...
#[tokio::test(flavor = "multi_thread")]
#[traced_test]
async fn e2e_test_rover_tag_assign_nonexistent_graph_id() {
...
#[tokio::test(flavor = "multi_thread")]
#[traced_test]
async fn e2e_test_rover_tag_assign_nonexistent_digest(remote_supergraph_graph_id: String) {
...
#[tokio::test(flavor = "multi_thread")]
#[traced_test]
async fn e2e_test_rover_tag_assign_happy_path(remote_supergraph_graph_id: String) {
    let tag = random_tag();
    info!("Assigning tag {tag} to artifact {E2E_TEST_ARTIFACT_DIGEST}");
