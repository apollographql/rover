...
/// publish mutation (returning a `launch.id`) and the launch status poll
/// query (returning `launch_status_response`), with the given extra CLI
/// args, and returns the process output. `--no-url` skips routing-url
/// determination entirely, so no routing-url-fetch mock is needed.
fn run_subgraph_publish(launch_status_response: &str, extra_args: &[&str]) -> std::process::Output {
...
fn subgraph_publish_reports_triggered_downstream_launches_in_text() {
    let output = run_subgraph_publish(LAUNCH_STATUS_RESPONSE, &[]);
...
fn subgraph_publish_reports_triggered_downstream_launches_in_json() {
    let output = run_subgraph_publish(LAUNCH_STATUS_RESPONSE, &["--format", "json"]);
...
fn subgraph_publish_fails_when_a_downstream_launch_fails() {
    let output = run_subgraph_publish(LAUNCH_STATUS_WITH_FAILED_DOWNSTREAM_LAUNCH_RESPONSE, &[]);
