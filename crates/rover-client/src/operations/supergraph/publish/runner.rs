...
#[graphql(
    query_path = "src/operations/supergraph/publish/supergraph_publish_mutation.graphql",
...
pub(crate) struct SupergraphPublishMutation;

pub async fn run(
    input: SupergraphPublishInput,
    client: &StudioClient,
) -> Result<SupergraphPublishResponse, RoverClientError> {
    ...
    if variant_exists {
        ...
        tracing::debug!(
            ...
            &input.subgraph_inputs[0].subgraph,
            &input.graph_ref
        );
    }

    let data = client.post::<SupergraphPublishMutation>(variables).await?;
...
fn build_response(publish_response: UpdateResponse) -> SupergraphPublishResponse {
    let build_errors: BuildErrors = publish_response
        ...
        .collect();
    tracing::debug!("Build errors found: {:?}", build_errors);
    ...
    fn build_response_works_with_composition_errors() {
        ...
        assert_eq!(
            ...
            SupergraphPublishResponse {
                ...
                subgraphs_created: Vec::new(),
                subgraphs_updated: Vec::new(),
                launch_url: None,
