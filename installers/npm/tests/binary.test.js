...
test('getBinary creates the passed directory if it doesn\'t exist', () => {
    let old_tmp_directories = fs.readdirSync(os.tmpdir(), {withFileTypes: true});
    ...
    let new_temp_directories = fs.readdirSync(os.tmpdir(), {withFileTypes: true});
...
test('install recreates an existing directory if it exists', () => {
    ...
    fs.writeFileSync(path.join(directory, "i-am-a-different-new-file.txt"), "binboobaznar")
    bin.install({}, true);
...
test('install downloads a binary if none exists', async () => {
    ...
    const bin= binary.getBinary(directory);
    //
    const directory_entries = await bin.install({}, true).then(
        async () => {
            return fs.readdirSync(directory, {withFileTypes: true})
        }
    );
    const filtered_directory_entries = directory_entries.filter(d => d.isFile() && d.name === `rover-${pjson.version}`);
    expect(filtered_directory_entries).toHaveLength(1);
    expect(fs.statSync(path.join(filtered_directory_entries[0].path, filtered_directory_entries[0].name)).size).toBe(0);
})
...
test('install renames binary properly', async () => {
    ...
    expect(directory_entries.filter(d => d.isFile() && d.name === `rover-${pjson.version}`)).toHaveLength(1);
})
...
test('install adds a new binary if another version exists', async () => {
    ...
    const directory_entries = fs.readdirSync(directory, {withFileTypes: true});
    expect(directory_entries.filter(d => d.isFile() && d.name === `rover-${pjson.version}`)).toHaveLength(0);
    ...
    // Check that we have 2 files of the form `rover-<<VERSION>>`
    expect(new_directory_entries.filter(d => d.isFile() && d.name.startsWith("rover-"))).toHaveLength(1);
    // Check that the new version got downloaded
    expect(new_directory_entries.filter(d => d.isFile() && d.name === (`rover-${pjson.version}`))).toHaveLength(1);
})
