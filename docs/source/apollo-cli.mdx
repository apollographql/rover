...
subtitle: Installation guide with npm
description: Learn about the partially deprecated Apollo CLI and how to install it.
---
...
Like all other tools, the Apollo CLI requires an API key to communicate with GraphOS. 
For each of your projects, [obtain a graph API key](/graphos/api-keys/#graph-api-keys) or [a subgraph API key](/graphos/api-keys/#subgraph-api-keys) for the project's associated graph or subgraph, and set that key as the value of `APOLLO_KEY` in your application's `.env` file:
