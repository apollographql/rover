...
Before publishing subgraph schema changes, preview the supergraph schema that composing (and, optionally, filtering) your subgraphs produces—without publishing anything.
...
Optional. The path to a YAML file describing hypothetical changes to one or more subgraphs to apply before composing. You can also provide `-`, in which case the command reads the file from `stdin`. For details, see the following [Previewing subgraph changes](#previewing-subgraph-changes).
...
Omit all of `--include-tag`/`--exclude-tag`/`--hide-unreachable-types` (and their `--no-*` counterparts) to preview composition only, with no filtering applied. If you provide any one of them, decide all three—use `--no-include-tags` to specify an empty include list.
...
Sets an empty include list for the previewed contract schema. To specify a non-empty list, use `--include-tag` instead. For details, see [contract filters](/graphos/delivery/contracts/#contract-filters).
...
Disable the automatic hiding of types that can never be reached in operations on the previewed contract schema.
...
Optional. Check the status of a previously started preview build a single time, instead of starting a new one. Do not combine with `--subgraph-changes`, `--async`, or the include/exclude/hide-unreachable-types options above.
...
If a preview build takes a long time, pass `--async` to start the build and return immediately with a build ID, instead of waiting until it completes:
...
By default (without `--async`), Rover polls the build's status until it completes or [`APOLLO_CHECKS_TIMEOUT_SECONDS`](../configuring/#supported-environment-variables) passes, whichever comes first. If Rover times out while polling, the build itself might still complete in the background—check back with `--build-id`.
