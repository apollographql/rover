...
You can similarly use [`rover subgraph publish`](#subgraph-publish) to push schema changes to a proposal.
...
#### Pushing schema changes to proposals

To push subgraph schema changes to a schema proposal rather than supergraph variant, use the proposal's ID instead of a variant name like so:

```bash
rover subgraph publish my-graph@p-101 --name accounts
```
...
Once you run the command, Rover confirms the proposal is updated and provides a GraphOS Studio link. That link is currently invalid, but as long as Rover confirms your proposal was updated, the changes are published to the proposal.
