...
Before publishing subgraph schema changes, preview the supergraph schema that composing (and, optionally, filtering) your subgraphs would produce—without publishing anything.
...
Omit all of `--include-tag`/`--exclude-tag`/`--hide-unreachable-types` (and their `--no-*` counterparts) to preview composition only, with no filtering applied. If you provide any one of them, decide all three—use `--no-include-tags` instead of this option to specify an empty include list.
...
Specifies an empty include list for the previewed contract schema. To specify a non-empty list, use `--include-tag` instead. For details, see [contract filters](/graphos/delivery/contracts/#contract-filters).
...
A tag name to [exclude tag names](/graphos/delivery/contracts/#contract-filters) when filtering. To exclude multiple tag names, specify `--exclude-tag` multiple times:
...
Specifies an empty [exclude list](/graphos/delivery/contracts/#contract-filters) for the previewed contract schema. Use `--exclude-tag` to specify a non-empty list.
...
Automatically [hide](/graphos/delivery/contracts/#contract-filters) types that can never be reached in operations on the previewed contract schema.
...
Disables the automatic hiding of types that can never be reached in operations on the previewed contract schema.
...
If a preview build might take a long time, pass `--async` to start the build and return immediately with a build ID, instead of waiting until it completes:
...
By default (without `--async`), Rover polls the build's status until it completes or [`APOLLO_CHECKS_TIMEOUT_SECONDS`](../configuring/#supported-environment-variables) elapses, whichever comes first. If Rover times out while polling, the build itself might still complete in the background—check back with `--build-id`.
