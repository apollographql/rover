...
subtitle: Generate and publish persisted query manifests
minVersion: Rover v0.17.2
description: Generate and publish persisted query manifests for safelisting and performance optimization by using the Rover CLI persisted-queries command.
---
...
1. Generating persisted query manifests (PQMs) from client operations
2. Publishing PQMs to the PQL using Rover

For more information on persisted query implementation steps, see the [GraphOS persisted queries documentation](/graphos/operations/persisted-queries).

## Generating a PQM

### `persisted-queries generate`

<MinVersionBadge version="Rover v0.41.0" />

Use `rover persisted-queries generate` to generate an Apollo PQM from `.graphql` operation files. This command runs locally and doesn't require GraphOS authentication.

```bash
rover persisted-queries generate
```

By default, Rover scans `graphql/**/*.graphql` from the current directory and writes `persisted-query-manifest.json`. This default pairs with `rover client extract`, which extracts operations from client source code into a `graphql` directory:
...
The generated manifest uses the Apollo persisted query manifest format:
...
`persisted-queries generate` supports multiple operations in a single `.graphql` file. It emits one manifest entry per operation, adds `__typename` fields using Apollo Client's default behavior, and generates each operation ID as the SHA-256 hash of the final operation body.
...
rover persisted-queries generate \
  --root-dir ./client \
  --include "operations/**/*.graphql" \
  --exclude "operations/**/*.test.graphql"
```
...
```bash
rover persisted-queries generate \
  --manifest-path ./dist/persisted-query-manifest.json
```

To get a machine-readable JSON response (path and operation count), use Rover's global `--format json` flag:

```bash
rover persisted-queries generate --format json
```
...
As with [generating manifests](/graphos/operations/persisted-queries#31-generate-persisted-query-manifests), it's best to execute this command in your CI/CD pipeline to publish new operations as part of your app release process. This command requires your API key to have the [**Graph Admin** or **Persisted Query Publisher** role](/graphos/platform/access-management/member-roles#graph-api-key-roles); the latter is a role specifically designed to work with this command without exposing unnecessary information about your graph to users of the API key.

#### Relay support

<MinVersionBadge version="Rover v0.19.0" />
