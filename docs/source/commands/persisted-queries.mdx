...
rover persisted-queries generate --manifest-path persisted-query-manifest.json
...
When you write the manifest to a file, add Rover's global `--format json` flag to get a machine-readable `{ path, operation_count }` response:

```bash
rover persisted-queries generate \
  --manifest-path persisted-query-manifest.json --format json
```

Without `--manifest-path`, `--format json` prints the manifest itself as JSON to stdout.
