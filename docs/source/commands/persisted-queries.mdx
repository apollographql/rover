...
Instead of `my-graph@my-variant`, you can also use the `--graph-id` and `--list-id` options to specify the PQL you want to publish to:

```bash
rover persisted-queries publish \
  --graph-id mygraph --list-id mylist \
  --manifest ./persisted-query-manifest.json
```

- Use `--graph-id` to provide the graph's unique identifier
- Use `--list-id` to provide the PQL's unique identifier

**The `persisted-queries publish` command does the following:**

1. It publishes all operations in the provided manifest file to the PQL linked to the specified variant, or to the specified PQL.
