...
If you delete a federated variant with this command, it also deletes all of that variant's subgraphs. To delete a single subgraph while preserving the variant, see [Deleting a subgraph](/rover/subgraphs/#deleting-a-subgraph).
