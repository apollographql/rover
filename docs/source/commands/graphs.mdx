...
You can run all configured checks before publishing via the `--check` option. This flag takes no value, and it runs all the standard checks configured for the given graph.

If any of the checks fail then the error message will be displayed, and no publish action will take place. If all the checks do pass then the publishing of the schema will proceed.

For example if you wanted to check a schema change for `test-graph` and then publish the same you could run two commands:
```shell
...
For more information on the `check` see the following section on `graph check`. Passing the `--check` option runs the same checks `rover graph check` would with the same arguments.
