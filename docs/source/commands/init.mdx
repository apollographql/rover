...
If you started with TypeScript, you also need to run the following command in a separate terminal window before running `rover dev`.
...
These start a subgraph server `rover dev` depends on. Learn more in the `getting-started.md` generated when you run `rover init`.
