...
Running `rover init` starts a short wizard that helps you create a new graph or MCP server.
...
For `rover init --mcp` options, see the [Initializing an MCP Server](#initializing-an-mcp-server) section below.
...
The `init` CLI wizard will walk you through the process of configuring and creating an MCP server, with MCP tools based on your GraphQL API operations. 
...
You can also pass options directly to `rover init --mcp` instead of answering prompts in the wizard. The wizard still asks for any information not provided via an option and will always ask for confirmation before creating any files locally.
...
When the `rover init` wizard is complete, you can run `rover dev` to start everything together:
...
There are more ways to run the MCP server - via Rover, Docker, Apollo Runtime Container or using the MCP server binary. 

Learn more about Rover `dev` [here](./dev), or read [Running the Apollo MCP Server](https://www.apollographql.com/docs/apollo-mcp-server/run) for the full guide on the different ways to run your MCP server.


#### Additional resources

- After running `rover init --mcp`, open the generated `MCP_README.md` file for next steps.
- To go further, check out the [Apollo MCP Server Quickstart](https://www.apollographql.com/docs/apollo-mcp-server/quickstart) for a full guide
- Review the [Config Reference](https://www.apollographql.com/docs/apollo-mcp-server/config-file) for deeper detail on how to work with Transport, Telemetry, CORS and more options. 
