...
<ExpansionPanel title="Experiencing issues?">

    - If you experience a crash, run `rover config clear` and then run `rover config auth` before running `rover init` again.

    - If you receive `error: Invalid API key found.` unset the `APOLLO_KEY` environment variable:
     ...
     Then, run `rover config auth` before running `rover init` again.
...
You can also pass options directly to `rover init` to skip the wizard in automated environments or scripts:

```terminal showLineNumbers=false
rover init --project-type <PROJECT_TYPE> --organization <ORGANIZATION> --project-use-case <PROJECT_USE_CASE> --project-name <PROJECT_NAME> --graph-id <GRAPH_ID>
```
...
| `--organization` | The ID of the organization where the graph should be created. | *(your org ID)* |
| `--project-use-case` | Helps preconfigure your project for specific patterns. | `connectors`, `graph-ql-template` |
| `--project-name` | The name for your local project setup. | *(your project name)* |
...
- Follow the instructions in the cloned repository's `getting-started.md`.
