...
> ⚠️ It is crucial that you match the `federation_version` with the version of the Router you are targeting. Refer to this [support table](https://www.apollographql.com/docs/router/federation-version-support) to safely roll out new composition versions.**

#### Auto-updates

If you do not specify a `federation_version` in `supergraph.yaml`, Rover determines which version to use according to the following logic:
...
The latest federation library version is stored in [this file]() on the main branch of the Rover repository. If you do not specify an exact federation version, new plugin versions will be delivered and sourced from this file. This auto-update flow will cause issues for you if you do not update your routers prior to your composition pipeline, which is why we recommend specifying an exact `federation_version` every time.
