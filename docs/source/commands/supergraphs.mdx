...
Whenever you run `rover supergraph compose`, Rover automatically downloads the compisition library for the given federation version you specified either in your [supergraph.yaml](#yaml-configuration-file) or via [stdin](#supergraph-compose).
...
> - If you specify a Federation 1 composition and any of your subgraphs uses a Federation 2 schema, composition fails.
...
If you _don't_ specify a `federation_version` in `supergraph.yaml` (which will be required in future versions), Rover determines which version to use according to the following logic:
