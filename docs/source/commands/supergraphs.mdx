...
The command composes with Federation 2, implemented by the [`@apollo/composition`](https://www.npmjs.com/package/@apollo/composition) library.
...
- Federation 1 is no longer supported. Specifying a Federation 1 version—`1`, `latest-1`, or an exact `=0.x.y`/`=1.x.y`—fails with an error. To migrate your subgraphs, see [Moving to Federation 2](/federation/federation-2/moving-to-federation-2).
- If you don't pin an exact `federation_version`, Rover composes with the latest release available at the time and warns you. A new federation release can change your supergraph schema without you choosing to, so pin an exact version for anything beyond local prototyping.

</Note>

#### Automatic updates

If you don't specify an exact `federation_version` in `supergraph.yaml`, Rover asks the Apollo plugin registry at `https://rover.apollo.dev` for the latest Federation 2 composition version each time it runs, so a new release is picked up as soon as it ships.
