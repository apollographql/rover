...
To preview how Rover resolves these references, use [`supergraph config expand`](#supergraph-config-expand).
...
Rover only supports Federation 2 composition, implemented by the [`@apollo/composition`](https://www.npmjs.com/package/@apollo/composition) library.

<Caution>

**Federation 1 is no longer supported.** Rover rejects a `federation_version` of `1` (or any Federation 1 version, such as `latest-0` in `rover install --plugin`) at every command that accepts one, including `rover supergraph compose`, `rover dev`, `rover lsp`, and `rover connector`. To keep composing, opt every subgraph in to Federation 2 by adding an `@link` directive—see [Moving to Federation 2](/federation/federation-2/moving-to-federation-2#opt-in-to-federation-2)—then remove any Federation 1 pin from your configuration.
...
flowchart TB;
  ...
  version--No-->optin("Does <strong>every</strong> subgraph schema<br/>opt in to Federation 2?");
  version--Yes-->usesetting("Use the version specified<br/>by <code>federation_version</code>");
  optin--No-->fed1err("Fails: opt in to Federation 2<br/>via <code>@link</code> first");
  optin--Yes-->fed2("Use latest Federation 2<br/>composition");
  class usesetting,fed1err,fed2 tertiary;
```
...
The concrete version behind `latest-2` is resolved at download time by Apollo's plugin-delivery service, not pinned in the Rover repository. If you don't specify an exact federation version, new plugin versions will be delivered automatically as they're released.
...
Your `supergraph.yaml` file can include [variable references](#variable-expansion) such as `${env.PRODUCTS_URL}` and `${file.path}`. To see how Rover resolves these references—without running composition—use `supergraph config expand`:

```bash
rover supergraph config expand --config ./supergraph.yaml
```

Rover prints the configuration with every reference expanded. This is useful for confirming that the correct environment variables and files are picked up before you compose. Pass `-` to `--config` to read the configuration from `stdin` instead of a file.

By default the expanded configuration is printed as YAML. Add `--format json` to receive it under an `expanded_config` field instead.