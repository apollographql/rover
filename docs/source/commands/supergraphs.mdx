...
You can optionally pass a variant's graphref to download each subgraph's SDL and compose the supergraph SDL like so:

```bash
rover supergraph config --graph-ref platform@staging
```

You can optionally pass a YAML configuration file (see below) to override specific subgraphs or add a new one. This is useful for testing new subgraph schema before publishing them.

```bash
rover supergraph config --graph-ref platform@staging --supergraph-config supergraph.yaml
```
