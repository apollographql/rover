...
The source variant must belong to the same graph as the contract variant. It must also be a federated variant with subgraphs. If your graph uses Federation 1, you must enable its support for the `@tag` directive in GraphOS Studio. Note that Rover no longer supports composing new Federation 1 supergraphs—this applies only to pre-existing Federation 1 variants.
