...
Before [publishing a contract](#publishing-a-contract-to-graphos), you can preview the contract schema that a given filter would produce, without publishing a contract variant.

Run the `contract preview` command, like so:
...
Specifies an empty [exclude list](/graphos/delivery/contracts/#contract-filters) for the previewed contract.
...
If provided, the preview automatically [hides](/graphos/delivery/contracts/#contract-filters) types that are unreachable from the contract schema's root fields.
...
**Optional.** Checks the status of a previously started preview build a single time, instead of starting a new one. Can't be combined with `--async` or the include/exclude/hide-unreachable-types options above.
