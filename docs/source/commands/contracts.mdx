...
Provide a tag name to [exclude](/graphos/delivery/contracts/#contract-filters) when filtering. To exclude multiple tag names, specify `--exclude-tag` multiple times:
...
Before [publishing a contract to GraphOS](#publishing-a-contract-to-graphos), you can preview the contract schema that a given filter would produce, without publishing a contract variant.
...
Specify an empty [include list](/graphos/delivery/contracts/#contract-filters) for your previewed contract.
...
Specifies an empty [contract filter list](/graphos/delivery/contracts/#contract-filters) for the previewed contract.
...
If provided, the preview automatically [hides](/graphos/delivery/contracts/#contract-filters) types that are unreachable from your contract schema's root fields.
...
**Optional.** Check the status of a previously started preview build a single time, instead of starting a new one. You can't combine this with `--async` or the include/exclude/hide-unreachable-types options above.
...
If a preview build might take a while, pass `--async` to start the build and return a build ID, instead of waiting until it completes:
