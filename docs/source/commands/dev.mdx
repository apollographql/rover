...
Apollo is actively improving the integration between Rover and GraphOS Studio. Get in touch if you'd like to share feedback on this feature.

</Note>


You can include a GraphOS Studio variant's graph ref with `rover dev` to use the variant's subgraphs in local development.

```bash title="Passing a graph ref"
rover dev --graph-ref platform@prod
```

(You can view a variant's subgraphs, including their routing URLs and schemas, from the **Subgraphs** page in Studio.)

<Note>

To pass a graph ref, you must have a personal API key or graph API key configured in Rover.

</Note>

When you include a graph ref, Rover uses the variant's subgraph routing URLs and schema in Studio as the [supergraph config](). This information is stored in memory and not saved to disk.

#### Overriding variant subgraphs

While developing locally, you may want to override subgraph(s) from your Studio variant with subgraphs from your local supergraph. You can do this by passing a supergraph config alongside a graph ref. Any subgraphs defined in the supergraph config override those from the graph ref.

For example, you can use a `supergraph_override.yaml` file that looks like this:
...
to override a variant's published `products` subgraph like so:

```
rover dev --graph-ref platform@prod --supergraph-config supergraph_override.yaml
```

 This command overrides (or adds, if there isn't a published `products` subgraph) in your local supergraph.
...
federation_version: =2.4.7
```
to override a variant's federation version like so:
