...
If you need to do things like modify CORS settings or modify header passthrough for subgraphs, you can do so by passing a valid [router configuration YAML file](https://www.apollographql.com/docs/router/configuration/overview#yaml-config-file) to `rover dev` via the `--router-config <ROUTER_CONFIG_PATH>` argument. Note that only the main `rover dev` process will use this router configuration file when starting the router. If you specify a different listen address with `supergraph.listen`, all other `rover dev` processes will need to pass the same values to `--supergraph-port` and `--supergraph-address` _or_ pass the same router configuration file path via `--router-config`. If you make changes to this file, the Router will recognize those changes and reload.
