...
Whenever you add, modify, or remove a subgraph from your local supergraph, Rover automatically re-composes your individual subgraph schemas into a unified supergraph schema, which your local router automatically begins using.
...
**When you start a `rover dev` session:**
...
3. Rover starts a locally running [router](/router) instance and provides it the supergraph schema.
...
After you start a session, you can then [add other subgraphs to it.](#adding-a-subgraph-to-a-session)

### Starting a session with multiple subgraphs

If you have a standard set of subgraphs that you're always developing with, you can create a [supergraph config file](./supergraphs#yaml-configuration-file) to add _all_ of them to your initial session with a single command.
...
If you do, a `rover dev` session starts with _one_ of the subgraphs listed, then adds the remaining subgraphs one at a time (order is undefined). Because of this, you might observe composition errors during intermediate steps.
...
After you start your initial `rover dev` session with a config file, you can still [add other subgraphs individually](#adding-a-subgraph-to-a-session). However, you _can't_ provide another config file.

## Adding a subgraph to a session

After you start a `rover dev` session with one subgraph, you can then add _other_ subgraphs to that same session.

To add a subgraph, open a new terminal window and run `rover dev` _again_, this time providing the details of the subgraph to add. For example, this command adds a `reviews` subgraph to the session started with a `products` subgraph above:
...
When you add a new subgraph to a session, Rover recomposes the supergraph schema and updates the router, so you can start querying _multiple_ subgraphs via the single router endpoint.
...
If you stop your _initial_ `rover dev` process (by pressing `CTRL+C`), it shuts down the local router instance and the entire session. This detaches and shuts down any _secondary_ `rover dev` processes.

## Stopping a single subgraph

If you stop a _secondary_ `rover dev` process (by pressing `CTRL+C`), the session recomposes its schema _without_ the removed subgraph and reloads the router.
