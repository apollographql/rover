...
You can also override a variant's federation version by passing the `--federation-version` option. This argument accepts fully-specified [federation versions](https://github.com/apollographql/federation/releases), prefixed with an equal sign (`=`), for example '=2.9.3'. Alternatively, you can use `=1` or `=2` for the latest release of Federation 1 or 2 respectively.
