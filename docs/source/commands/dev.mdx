...
description: Use the Rover CLI dev command to run your federated GraphQL API or MCP server in your local development environment. Learn how to start a router session, add and remove subgraphs, MCP tools, and more.
---

# Running Your Supergraph
...
There are multiple ways to run the [Apollo MCP Server](https://www.apollographql.com/docs/apollo-mcp-server) - `rover dev --mcp` being one of them. You can also run the Apollo MCP Server via Docker, Apollo Runtime Container or using the MCP server binary. 

### `rover dev --mcp`

You can use the Rover CLI's rover `dev` command to run the MCP server locally, alongside your GraphQL API. Use the `--mcp` flag to start an MCP server and provide an optional configuration file:
...
This will start up and serve your Graph API and MCP server as two local endpoints:
