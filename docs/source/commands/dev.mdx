...
description: Iterate on multiple subgraphs
---

This Rover command allows you to federate one or more running subgraphs into a supergraph that can be queried by any GraphQL client.
...
Before starting `rover dev`, you need to start your subgraph server. Note the URL it is running on, and the location of the schema on the filesystem if it exists. If you don't have a local schema file, introspection will need to be enabled on the subgraph.
...
If you do not pass the `--schema <PATH>` argument, `rover dev` will introspect the URL that was specified by the `--url <SUBGRAPH_URL>` argument once every second. The command prefers to use the same query as `rover subgraph introspect`, but it will fall back to `rover graph introspect` if your subgraph does not support introspecting federated SDL. Note that in this case directives will be stripped from the subgraph schema.
