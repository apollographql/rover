...
description: Use the Rover CLI dev command to run your federated Graph in your local development environment. Learn how to start a router session, add and remove subgraphs, and more.
---

A _supergraph_ is an architecture consisting of multiple Graphs (_subgraphs_) and a _graph router_ that runs in front of them:
...
To use `rover dev`, you need at least one running Graph (subgraph). Rover can obtain a subgraph's schema via introspection (either standard or [federated introspection](/federation/subgraph-spec#enhanced-introspection-with-query_service)), or you can provide a local schema file.
