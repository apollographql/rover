...
Note that only the main `rover dev` process uses this router configuration file when starting the router. If you specify a different listen address with `supergraph.listen`, all other `rover dev` processes need to pass the same values to `--supergraph-port` and `--supergraph-address`, _or_ pass the same router configuration file path via `--router-config`. If you make changes to this file, the router recognizes those changes and reloads.
