...
You can also override a variant's federation version by passing the `--federation-version` option. This argument accepts fully qualified [federation versions](https://github.com/apollographql/federation/releases), prefixed with an equals sign (`=`), for example '=2.9.3'. Alternatively, you can use `2` (without an equals sign) for the latest release of Federation 2.
...
By default, `rover dev` asks the Apollo plugin registry at `https://rover.apollo.dev` for the latest version of the router, composition, and MCP server plugins. You can override any of these versions:

- Router: Set the `APOLLO_ROVER_DEV_ROUTER_VERSION` environment variable (e.g., `APOLLO_ROVER_DEV_ROUTER_VERSION=1.0.0`)
- Composition: Use the `--federation-version` flag or set the `APOLLO_ROVER_DEV_COMPOSITION_VERSION` environment variable (e.g., `--federation-version '=2.0.0'` or `APOLLO_ROVER_DEV_COMPOSITION_VERSION=2.0.0`). The flag takes the `supergraph.yaml` spelling, so an exact version needs the `=` prefix; the environment variable takes a bare version.
