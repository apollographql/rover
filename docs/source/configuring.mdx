...
[Learn how to obtain an API key.](/graphos/platform/access-management/api-keys)
...
When an error has underlying causes (the `Caused by:` lines shown in plain-text output), a `causes` field is included with an array of those cause messages, outermost first.
...
You can configure Rover's behavior by setting the environment variables listed below. Most have a corresponding flag, shown in the table; a flag's `--help` text also names its environment variable.

For a setting that has both a flag and an environment variable, the flag takes precedence when both are supplied. For a setting that's environment-variable-only, its value is used whenever it's set, overriding Rover's built-in default.

<Note>

This is separate from [API key resolution](#2-provide-the-api-key-to-rover), which has its own precedence and isn't affected by the above.

</Note>

| Name                        | Flag | Value          |
|-----------------------------|------|----------------|
| `APOLLO_HOME` | `--rover-home` | The path to the parent directory of Rover's binary. The default value is your operating system's default home directory. Rover will install itself in a folder called `.rover` inside the directory specified. |
| `APOLLO_CONFIG_HOME` | `--config-home` | The path where Rover's configuration is stored. The default value is your operating system's default configuration directory. |
...
| `APOLLO_TELEMETRY_DISABLED` | `--telemetry-disabled` | Set to `true` if you don't want Rover to collect anonymous usage data. Unlike most boolean settings, any value (including `false`) disables telemetry — only its absence leaves it enabled. |
...
| `APOLLO_CHECKS_TIMEOUT_SECONDS` | `--checks-timeout` | Overrides how long, in whole seconds, `check`/`publish` polling waits before giving up. Defaults to `300`. |
| `APOLLO_CLIENT_TIMEOUT` | `--client-timeout` | Overrides the timeout (in seconds) for HTTP(S) requests. See [Increasing request timeouts](#increasing-request-timeouts). |
| `APOLLO_ROVER_DOWNLOAD_HOST` | `--download-host` | Overrides the host plugin binaries (the `router` and `supergraph` composition plugins) are downloaded from. |
| `APOLLO_TEMPLATES_API` | `--templates-api` (`rover init`/`rover template` only) | Overrides where `rover init`/`rover template` fetch project templates from. |
...
| `APOLLO_VCS_REMOTE_URL` | `--vcs-remote-url` | The URL of your project's remote repository. See [Git context](#git-context). |
| `APOLLO_VCS_BRANCH` | `--vcs-branch` | The name of the version-controlled branch. See [Git context](#git-context). |
| `APOLLO_VCS_COMMIT` | `--vcs-commit` | The long identifier (SHA in Git) of the commit. See [Git context](#git-context). |
| `APOLLO_VCS_AUTHOR` | `--vcs-author` | The name and email of a commit's author (e.g., `Jane Doe <jane@example.com>`). See [Git context](#git-context). |
| `APOLLO_OAUTH_AUTHORIZATION_URL` | `--oauth-authorization-url` | Overrides the OAuth authorization endpoint `rover auth login` uses. |
| `APOLLO_OAUTH_TOKEN_URL` | `--oauth-token-url` | Overrides the OAuth token endpoint `rover auth login` uses. |
| `APOLLO_OAUTH_WHOAMI_URL` | `--oauth-whoami-url` | Overrides the OAuth whoami/userinfo endpoint `rover auth whoami` uses. |
| `APOLLO_OAUTH_CLIENT_ID` | `--oauth-client-id` | Overrides the OAuth client ID `rover auth login` uses. |
...
| `APOLLO_LOG_LEVEL` | `--log` | Sets Rover's [log level](#logging). |
| `APOLLO_FORMAT` | `--format` | Sets Rover's [output format](#configuring-output) (`plain` or `json`). |
| `NO_COLOR` | `--no-color` | Set to any value other than an empty string, `0`, or `false` (case-insensitive) if you don't want Rover to print color. |
| `APOLLO_NO_COLOR` | `--no-color` | Same as `NO_COLOR`, with the same value rule. Checked in addition to, not instead of, `NO_COLOR`. |
