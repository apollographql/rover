...
All Rover commands that communicate with [GraphOS](/graphos/) require an API key to do so. GraphOS supports three types of API keys: _personal API keys_, _subgraph API Keys_, and _graph API keys_.

- **On your local development machine,** use a personal API key.
- **In team-specific CI** use a subgraph API key
- **In all other shared environments** use a graph API key.

[Learn how to obtain an API key.](/graphos/api-keys/)

### 2. Store the API key

Store the key's token value and name in your secret manager of choice. 

### 3. Provide the API key to Rover
...
You can create multiple _configuration profiles_ in Rover. Each configuration profile has its own associated API key, so you can use different configuration profiles when interacting with different graphs or subgraphs.
