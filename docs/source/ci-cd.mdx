    ...
    steps:
      - run:
          ...
          command: |
            ...
            curl -sSL https://rover.apollo.dev/nix/v0.30.0 | sh
...
- Download the latest release via cURL and extract the binary like so (this downloads Rover `0.27.0` for Linux x86 architectures):
    ...
    ```
    curl -L https://github.com/apollographql/rover/releases/download/v0.30.0/rover-v0.30.0-x86_64-unknown-linux-gnu.tar.gz | tar --strip-components=1 -zxv
    ```
