...
On your local development machine, you can use a personal API key, and provide the key to Rover using the [`rover config auth` command](./configuring#via-the-auth-command).

In shared environments like CI, use a graph or subgraph API key. Because the <code>rover config auth</code> command is interactive, it isn't suitable for CI/CD. Instead, [authenticate using an environment variable](./configuring#with-an-environment-variable).
