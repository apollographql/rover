    ...
    steps:
      - run:
          ...
          command: |
            ...
            curl -sSL https://rover.apollo.dev/nix/v0.36.1 | sh
...
- Download the latest release via `cURL` and extract the binary. This downloads Rover `0.36.1` for Linux x86 architectures:

  ```
  curl -L https://github.com/apollographql/rover/releases/download/v0.36.1/rover-v0.36.1-x86_64-unknown-linux-gnu.tar.gz | tar --strip-components=1 -zxv
  ```
