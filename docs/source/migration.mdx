...
If you need introspection JSON (`{ "__schema": ... }`) instead of SDL—for example, to replace `apollo service:download` or `apollo schema:download`—pass `--introspection-json` to `graph introspect`. See [`graph introspect`](/rover/commands/graphs#graph-introspect) for details.
...
## Rover ##
# use --introspection-json for the legacy JSON format; default output is SDL
rover graph introspect http://localhost:4000 --introspection-json --output schema.json
```
