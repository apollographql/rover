...
description: Learn about the partially deprecated Apollo CLI and how to transition to the Rover CLI for interacting with Apollo GraphOS services, including replacing `service:download` with introspection JSON output.
...
If you need introspection JSON (`{ "__schema": ... }`) instead of SDL—for example, to replace `apollo service:download` or `apollo schema:download`—use `--format json` with `graph introspect`. See [`graph introspect`](/rover/commands/graphs#graph-introspect) for details.
...
```bash
## Apollo CLI ##
# outputs introspection JSON
apollo service:download --endpoint http://localhost:4000

## Rover ##
# use --format json for introspection JSON; default output is SDL
...
### Validate client operations against a schema

```bash
## Apollo CLI ##
apollo client:check --variant=staging
...
### Extract embedded GraphQL from source files

```bash
## Apollo CLI ##
apollo client:extract --variant=staging --outputFile operations.json
