...
If you use Rover, use [`rover client check`](./commands/client#client-check) instead of `apollo client:check`. The Apollo CLI command documented on this page is still supported but is the legacy path.
