...
If you're using Rover, use [`rover client check`](./commands/client#client-check) instead of `apollo client:check`. The Apollo CLI command documented below is still supported but is the legacy path.
