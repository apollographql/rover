...
- **FR7**: Every plugin-using command must accept one version flag, taking the FR1 grammar, for each plugin in its chain, and must not accept a version flag for a plugin it never runs. The flag surface is therefore determined by the command's plugin chain, not chosen per command:
  ...
  | Command | Plugins in its chain | Version flags |
  |---|---|---|
  | `rover supergraph compose` | `supergraph` | `--federation-version` |
  | `rover connector` | `supergraph` | `--federation-version` |
  | `rover lsp` | `supergraph` | `--federation-version` |
  | `rover dev` | `supergraph`, `router`, `apollo-mcp-server` | `--federation-version`, `--router-version`, `--mcp-version` |

  `rover dev --router-version` and `rover lsp --federation-version` are new; the other flags exist today. Each flag has an environment variable equivalent that every command in that flag's rows must honor, with the flag winning when both are set: `APOLLO_ROVER_DEV_COMPOSITION_VERSION` for `--federation-version`, `APOLLO_ROVER_DEV_ROUTER_VERSION` for `--router-version`, and `APOLLO_ROVER_DEV_MCP_VERSION` for `--mcp-version`. The `_DEV_` in those names is historical: today only `rover dev` reads them, and the names are kept for compatibility even though `compose`, `connector`, and `lsp` now read the composition one too (see FR86). A version flag for a plugin outside the command's chain, such as `rover supergraph compose --router-version 2`, must be rejected as an unrecognized argument before any manifest is read or any connection is made.
...
- **FR29**: For each plugin a command needs, Rover must determine the version request by taking the first of these that is present, most specific first:
  1. An explicit command-line argument: the command's version flag for that plugin from the FR7 table, or a `rover plugin install` positional `<name>@<version>`.
  2. The corresponding environment variable from FR7, for every command whose chain includes that plugin.
...
- **FR86**: The three version environment variables must be documented in one place as applying to every command in their FR7 rows, with an explicit note that the `_DEV_` in their names is historical and does not mean they are limited to `rover dev`. Each command's own documentation and `--help` text for a version flag must name its environment variable equivalent.
...
**Version flags follow the plugin chain**
Given any plugin-using command, when it is invoked with `--help`, then it lists exactly the version flags in its FR7 row. Given `rover supergraph compose --router-version 2`, when it runs, then it fails as an unrecognized argument, no manifest is read, and no connection is made.

**`rover lsp` accepts a federation version**
Given `supergraph.yaml` with `federation_version: =2.9.3`, when `rover lsp --federation-version =2.8.0` starts, then composition inside the session uses `2.8.0`, exactly as the same flag behaves on `rover supergraph compose`.

**The composition environment variable is not `dev`-only**
Given `supergraph.yaml` with `federation_version: =2.9.3` and `APOLLO_ROVER_DEV_COMPOSITION_VERSION=2.8.0`, when `rover supergraph compose` runs, then `2.8.0` is used. Given the same environment plus `--federation-version =2.7.0`, then `2.7.0` is used.
