...
6. **`--profile` becomes a global flag.** Every command accepts it, and Rover resolves it once from the parsed command line before any command runs, so whether the user passed it is knowable. Part A's override notices and all of Part B depend on that distinction; this slice delivers it.
...
- **Parity is the default, not an absolute.** Three flags stay flag-only on purpose (`--profile` and the two insecure TLS flags, see P2). In all three cases the setting changes where requests go or how they are trusted, and a value inherited from the shell would make that invisible at the point of use. Containers and CI images that want them on every invocation put them in the entrypoint, which keeps them visible in the image definition. `--output` is also unpaired, for a different reason: it names this invocation's output file, which is not configuration, so it falls outside P1.1 rather than being an exception to it.
...
6. **`rover config` covers settings, not just credentials.** Users can set, read, and unset individual settings on a profile through `rover config`, with values validated syntactically at write time so a malformed URL or non-numeric timeout is rejected when entered. Settings live under `rover config`; existing credential verbs (`rover config auth`, `rover auth login|logout`) are unchanged and no new credential verbs are added.
...
- **Delivery slicing.** Slice one: settings storage, the inspection verb, notices, and the registry/telemetry/OAuth-endpoint group; it assumes the Prerequisite's `--profile` distinguishability rather than delivering it. Later slices: VCS context, graph ref, checks timeout, download host, templates API.
...
Depends on Part A. Introduces a committable artifact and extends the precedence rule.
...
3. **Project-file values apply as-is.** A network-destination value from the project file is used exactly like one from a profile: no confirmation step, no allowlist, no per-user trust record. The same is true interactively and in CI.
4. **Surfacing extends to project files.** A1.5(b) also fires when the project file supplies a non-default network-destination value, and it is the receipt for B1.3: every command that sends a request to a project-file-supplied destination says so once.

## B2. Key decisions and rationale

- **Explicitly selected profile beats the project file.** When a user types `--profile staging`, they have named the environment they want for this invocation. Letting a file in the working tree silently override that reproduces the env-var footgun with no CI justification to excuse it. `--profile default` typed literally is explicit, which is the user's escape from a repo's settings. The implicit default profile sits *below* the project file because omitting `--profile` expresses no intent, and a team's committed settings should win over a user's unstated default. An explicit profile outranking a project-file value is not an override-notice case: the user named the profile, so there is nothing to surface. A1.5(a) is for an env var overriding a profile, and only that.
...
- **Network destinations from the project file apply as-is.** A project file arrives via `git clone`, so a repository can point the registry URL or download host anywhere, and Rover will use it. That is accepted, for three reasons. The B1.4 notice names the setting and its value on every command that sends a request there, so the redirect is never silent. A flag, env var, or explicitly selected profile outranks the file, so a user or pipeline that wants a fixed destination sets it once and the file cannot move it. And plugin binaries fetched from a redirected download host remain subject to ROVER-420's checksum verification once the registry publishes checksums. Making network-destination settings ineligible for project files was rejected because "our repo uses our mirrored registry" is a primary reason teams want a project file. Rejected, and recorded so it can be reopened: a trust prompt keyed on the set of network-destination values, asked once per user and re-asked on any change. It only protects interactive users, since a pipeline cannot be prompted and would need a blanket grant, which is no gate at all; it adds a trust store and `rover config` verbs to manage it; and the notice already surfaces the same fact at the moment it matters.
- **Honest CI accounting.** For a CI pipeline that today sets several `APOLLO_*` env vars for settings, the project file replaces all of them. The pipeline still supplies its credential (`APOLLO_KEY` or the client-credentials pair), which the project file cannot carry.
...
- **One file, two owners.** The manifest's plugin keys (`plugins:`, `install_root`) belong to ROVER-420; `settings:` belongs to this PRD. Each owner's rules apply only to its own section: this PRD's validation and surfacing rules cover `settings:` and nothing else, while the plugin section is protected by that spec's checksum and install-root rules. Any Rover command that writes the manifest (today, `rover plugin install` recording what it installed) must preserve the other owner's section exactly as the author left it. The two precedence chains, plugin version resolution and settings resolution, stay independent; the sections are disjoint, so there is nothing to unify. One interaction is worth naming: `APOLLO_ROVER_DOWNLOAD_HOST` in `settings:` redirects where plugin binaries come from. The B1.4 notice surfaces that redirect, and ROVER-420's checksum verification is the integrity control on what arrives.
...
- **Problem 2.** A repo containing a project file and no other configuration produces identical value/source pairs for every eligible setting for two different users on two different machines, verified by the inspection verb's JSON output.
...
- **Surfacing.** In a fresh clone whose project file sets a download host, running local composition contacts that host, emits exactly one notice on stderr naming the setting and its value, and leaves the exit code unchanged. Setting the suppression env var silences the notice and changes nothing else.
- **Explicit profile wins.** In a repo whose project file sets a registry URL, `rover graph check --profile staging` uses staging's registry URL when staging defines one. The only notice emitted is the A1.5(b) notice for the profile-supplied destination; nothing mentions the project-file value it outranked.
