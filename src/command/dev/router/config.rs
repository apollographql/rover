...
impl RouterConfigReader {
            ...
            if Fs::assert_path_exists(input_config_path).is_err() {
               ...
               (ip, port, config)
                ...
                let ip = self
                    .ip_override
                    .clone()
                    .unwrap_or_else(|| yaml_ip.unwrap_or(DEFAULT_ROUTER_ADDRESS.to_string()));
                let port = self
                    .port_override
                    .map(|p| p.to_string())
                    .unwrap_or_else(|| yaml_port.unwrap_or_else(|| DEFAULT_ROUTER_PORT.to_string()));
    ...
    fn get_config_from_opts(&self) -> (String, String, String) {
        let ip = self.ip_override.clone().unwrap_or(DEFAULT_ROUTER_ADDRESS.to_string());
        let port = self.port_override.unwrap_or(DEFAULT_ROUTER_PORT).to_string();
        let config = format!(
