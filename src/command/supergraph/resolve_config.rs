...
use rover_client::{r#async::GraphQLClient, RoverClientError};
...
pub(crate) fn resolve_supergraph_yaml(
    ...
    rt.block_on(async {
        ...
        let subgraph_definition_results: Vec<(String, RoverResult<SubgraphDefinition>)> =
            futures::stream::iter(supergraph_config
            .iter()
            .map(|(subgraph_name, subgraph_data)| {
                async { let cloned_subgraph_name = subgraph_name.to_string();
                let result = match &subgraph_data.schema {
                        ...
                        SchemaSource::SubgraphIntrospection {
                            ...
                            resolve_subgraph_introspection(
                                client_config.clone(),
                                subgraph_url,
                                introspection_headers,
                                subgraph_name,
                        ...
                        SchemaSource::Sdl { sdl } => resolve_sdl(subgraph_name, subgraph_data, sdl).await,
                };
            ...
            .collect::<Vec<(String, RoverResult<SubgraphDefinition>)>>().await;
...
) -> RoverResult<SubgraphDefinition> {
...
async fn resolve_subgraph(
    ...
    let authenticated_client = client_config
        .get_async_authenticated_client(profile_opt)
        .map_err(RoverError::from)?;
    // given a graph_ref and subgraph, run subgraph fetch to
    // obtain SDL and add it to subgraph_definition.
    let result = fetch::run_async(
        SubgraphFetchInput {
...
fn err_no_routing_url() -> RoverError {
