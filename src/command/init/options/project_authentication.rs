        ...
        let api_key = match password_result {
            Ok(input) => {
                ...
                if !input.trim().starts_with("user:") {
                    return Err(auth_error_to_rover_error(AuthenticationError::InvalidKeyFormat));
                }
                
                input
            }
            Err(e) => return Err(auth_error_to_rover_error(AuthenticationError::SystemError(e.to_string()))),
        };
