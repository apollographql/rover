...
mod tests {
    ...
    fn test_auth_error_types_convert_to_appropriate_messages() {
        ...
        assert!(validation_error.to_string().contains("Invalid API key format"));
        ...
        assert!(credential_error.to_string().contains("Invalid API key found"));
        
        // System/infrastructure errors
        let system_error = auth_error_to_rover_error(AuthenticationError::SystemError("db connection".to_string()));
    ...
    fn test_credential_errors_guide_to_resolution() {
        ...
        let auth_failed = auth_error_to_rover_error(AuthenticationError::AuthenticationFailed("invalid".to_string()));
    ...
    fn test_system_errors_guide_to_support() {
        let system_error = auth_error_to_rover_error(AuthenticationError::SystemError("unexpected".to_string()));
    ...
    impl ApiKey {
        ...
        fn parse(key: &str) -> Self {
            if key.is_empty() {
                return ApiKey { key: key.to_string(), key_type: KeyType::Invalid };
            }
            
            if key.starts_with("user:") {
                ApiKey { key: key.to_string(), key_type: KeyType::User }
            } else if key.starts_with("graph:") {
                ApiKey { key: key.to_string(), key_type: KeyType::Graph }
            } else {
                ApiKey { key: key.to_string(), key_type: KeyType::Invalid }
            }
        }
