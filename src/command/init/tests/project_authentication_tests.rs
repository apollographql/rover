...
mod tests {
    ...
    impl AuthWorkflowSimulation {
        ...
        fn enter_key(&mut self, key: &str) {
            ...
            match key {
                ...
                "user:invalid_credentials" => {
                    self.error = Some(AuthenticationError::AuthenticationFailed("Invalid credentials".to_string()));
                },
                _ => {
                    // Simulate a network or system error
                    if key.contains("system_error") {
                        self.error = Some(AuthenticationError::SystemError("Database error".to_string()));
                    } else {
                        // Default to invalid credentials
                        self.error = Some(AuthenticationError::AuthenticationFailed("Unknown error".to_string()));
                    }
                }
            }
        }
        ...
        fn get_user_message(&self) -> String {
            ...
            match &self.error {
                Some(err) => {
                    let rover_error = crate::command::init::authentication::auth_error_to_rover_error(err.clone());
    ...
    fn test_successful_authentication_flow() {
        ...
        assert_eq!(workflow.get_user_message(), "Successfully saved your API key.");
    }
    ...
    fn test_empty_key_authentication_flow() {
        ...
        assert!(workflow.get_user_message().contains("API key cannot be empty"));
    }
    ...
    fn test_invalid_format_authentication_flow() {
        ...
        assert!(workflow.get_user_message().contains("Invalid API key format"));
    }
    ...
    fn test_graph_key_authentication_flow() {
        ...
        assert!(workflow.get_user_message().contains("Invalid API key found"));
    }
    ...
    fn test_invalid_credentials_authentication_flow() {
        ...
        assert!(!workflow.authenticated);
        assert!(matches!(workflow.error, Some(AuthenticationError::AuthenticationFailed(_))));
        assert!(workflow.get_user_message().contains("Invalid API key found"));
    }
    ...
    fn test_system_error_authentication_flow() {
        ...
        assert!(!workflow.authenticated);
        assert!(matches!(workflow.error, Some(AuthenticationError::SystemError(_))));
        assert!(workflow.get_user_message().contains("Unexpected system error"));
    }
} 