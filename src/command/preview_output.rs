use rover_client::shared::{AsyncBuildStatus, PreviewJobResponse, PreviewKind};
use rover_std::Style;

use crate::command::CliOutput;

/// [`CliOutput`] implementation shared by `rover contract preview` and
/// `rover subgraph preview`. Both commands produce the same
/// [`PreviewJobResponse`] shape and differ only in which command name to
/// show in the "check back later" hint, which [`PreviewJobResponse::kind`]
/// already carries.
#[derive(Debug)]
pub(crate) struct PreviewOutput(pub(crate) PreviewJobResponse);

impl CliOutput for PreviewOutput {
...
mod tests {
    ...
    fn response(status: AsyncBuildStatus, kind: PreviewKind) -> PreviewJobResponse {
        PreviewJobResponse {
    ...
    fn exit_code_is_zero_for_in_progress_and_success_statuses() {
        for status in [
            ...
            assert_eq!(
                PreviewOutput(response(status, PreviewKind::Contract)).exit_code(),
                0
            );
        }
    }
    ...
    #[test]
    fn text_for_pending_hints_the_right_command_per_kind() {
        let text = temp_env::with_var("NO_COLOR", Some("1"), || {
            PreviewOutput(response(AsyncBuildStatus::Pending, PreviewKind::Subgraph)).text()
        });
        assert!(text.contains("rover subgraph preview my-graph@current --build-id build-123"));

        let text = temp_env::with_var("NO_COLOR", Some("1"), || {
            PreviewOutput(response(AsyncBuildStatus::Pending, PreviewKind::Contract)).text()
        });
    ...
    fn text_for_success_includes_the_schema() {
        let mut mock_response = response(AsyncBuildStatus::Success, PreviewKind::Contract);
        mock_response.api_schema = Some("type Query { hello: String }".to_string());
        let text = PreviewOutput(mock_response).text();
        assert!(text.contains("type Query { hello: String }"));
    }

    #[test]
    fn text_for_failure_includes_the_errors() {
        let mut mock_response = response(AsyncBuildStatus::ComposeFailed, PreviewKind::Contract);
    ...
    fn json_serializes_the_response() {
        let json = PreviewOutput(response(AsyncBuildStatus::Success, PreviewKind::Contract))
            .json()
            .unwrap();
