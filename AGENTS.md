...
- **Compose retry and timeout explicitly for network-calling commands and operations; don't rely on the ambient client timeout alone.** Every `reqwest::Client` built through `StudioClientConfig`/`ClientBuilder` (`src/utils/client.rs`) already carries a floor timeout from the global `--client-timeout` flag (default 30s), and `StudioClient::studio_graphql_service()` (`crates/rover-client/src/blocking/studio_client.rs`) bakes a `RetryLayer` into the ~10 GraphQL operations that call it via `runner.rs`. That's a partial, inconsistent baseline: it has no per-attempt `TimeoutLayer`, and commands that go through `StudioClientConfig::service()`/`authenticated_service()` directly (e.g. `src/command/client/check/mod.rs`, `src/composition/*`, `src/command/dev/*`, `src/command/lsp/mod.rs`) get neither retry nor an explicit Tower timeout — only the bare reqwest-level floor. When adding or touching a network-calling operation or command, compose `rover-http`'s `RetryLayer`/`TimeoutLayer` explicitly via `ServiceBuilder`, following `src/command/auth/whoami/mod.rs`'s pattern (mirrored in `src/cli.rs`'s client-credentials resolver): a short per-attempt `TimeoutLayer` nested inside a `RetryLayer` whose retry budget (`client_config.retry_period()`) is the longer elapsed-time window, so a single hung attempt can't consume the entire retry budget. Don't reach for tower's own built-in `ServiceBuilder::timeout()` (as `src/command/auth/logout.rs` does) — use `rover-http`'s `TimeoutLayer` so timeout behavior stays consistent and greppable across the codebase. When reviewing a PR that adds or touches a network-calling command or operation, flag it if it relies solely on the ambient `--client-timeout` floor with no explicit retry/timeout composition, or if it introduces a new ad hoc timeout/retry mechanism instead of reusing `rover-http`'s layers.
...
- **Use `insta` snapshot tests to verify JSON output structure, including in `tests/e2e`.** `tests/integration` already does this well — `assert_json_snapshot!`/`assert_snapshot!` against checked-in `.snap` files (`tests/integration/client/extract.rs:149`, `tests/integration/schema/search.rs:181-261`; see `tests/README.md:77-86` for the `cargo insta review`/`INSTA_UPDATE=always` workflow). Extend the same pattern to `tests/e2e` when adding or touching e2e coverage of a command's JSON output — don't hand-roll ad hoc field-by-field assertions there instead.
- **Use `rstest` for fixtures, `speculoos` for assertions — both are already the dominant style; keep it that way.** See `tests/README.md` for existing fixture conventions. When asserting with `speculoos`'s `assert_that!`, assert on the full object or the full string (`.is_equal_to(...)` against a complete expected value), not just `.is_ok()`/`.is_some()`/`.contains(...)`. A shallow assertion (result is `Ok`, string contains a substring) passes even when the rest of the value silently regresses; `src/command/auth/whoami/output.rs:64-92` (asserting the entire rendered table and entire JSON value) is the model to follow.
