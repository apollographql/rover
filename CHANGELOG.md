...
- **`rover graph publish` polls a triggered launch (and its downstream contract-variant launches) to completion, and `--format json` gains `launch_url`/`launch_status`/`downstream_launches` fields - @dotdat**

  Previously, `graph publish` returned as soon as the schema upload succeeded, with no visibility into whatever launch that publish triggered. It now polls the launch (and any downstream contract-variant launches it triggered) until each reaches a final status, or `APOLLO_CHECKS_TIMEOUT_SECONDS` elapses, extending how long a publish that triggers a launch takes to return. `--format json` output gains `launch_url`, `launch_status`, and `downstream_launches` fields reflecting the outcome; text/stderr output doesn't report on them yet — that's a following change in this stack.
